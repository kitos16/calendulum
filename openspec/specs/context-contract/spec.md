...
### Requirement: template context exposes date, inMonth, isToday, isSelected

Every day template outlet (`dayCell`, `dayCellTop`, `dayCellBottom`) MUST provide an implicit
context value exposing `date: Date`, `inMonth: boolean`, `isToday: boolean`, and
`isSelected: boolean`. The flags MUST reflect current component state at render time. The
exported `CalendulumDayCellContext` type MUST describe this shape.
...
README MUST document the context as `{ date, inMonth, isToday, isSelected }` — naming
`isSelected`, not `selected` — matching the exported type exactly.
